...
public abstract class AReactor extends SlimefunItem implements RecipeDisplayItem {
	...
	private void constructMenu(BlockMenuPreset preset) {
		...
		preset.addItem(1, new CustomItem(SlimefunItems.URANIUM, "&7Топливный слот", "", "&rПринимает радиоактивное топливо:", "&2уран &rили &aнептуний"), ChestMenuUtils.getEmptyClickHandler());
		...
		if (needsCooling()) {
			...
			for (int i : border_4) {
				preset.addItem(i, new CustomItem(new ItemStack(Material.BARRIER), "&cОхлаждение не требуется"), ChestMenuUtils.getEmptyClickHandler());
			}
		}
	}
