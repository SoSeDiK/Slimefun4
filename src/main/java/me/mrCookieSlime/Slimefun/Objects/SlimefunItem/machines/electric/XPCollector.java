	...
	protected void tick(Block b) {
		...
		while (iterator.hasNext() && xp == 0) {
			...
			for (int level = 0; level < getEXP(b); level = level + 10) {
				if (menu.fits(new CustomItem(Material.EXPERIENCE_BOTTLE, "&aНастой знаний"), getOutputSlots())) {
					withdrawn = withdrawn + 10;
					menu.pushItem(new CustomItem(Material.EXPERIENCE_BOTTLE, "&aНастой знаний"), getOutputSlots());
				}
			}
